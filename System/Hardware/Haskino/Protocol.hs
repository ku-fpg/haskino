...
packageExpr (Ref8 n) = packageRef n (exprCmdVal EXPR_WORD8 EXPR_REF)
...
packageExpr (Ref16 n) = packageRef n (exprCmdVal EXPR_WORD16 EXPR_REF)
...
packageExpr (Ref32 n) = packageRef n (exprCmdVal EXPR_WORD32 EXPR_REF)
...
parseQueryResult (RemoteBinding (NewRemoteRefB _)) (NewReply r) = Just $ RemoteRefB $ RefB $ fromIntegral r
parseQueryResult (RemoteBinding (NewRemoteRef8 _)) (NewReply r) = Just $ RemoteRefW8 $ Ref8 $ fromIntegral r
parseQueryResult (RemoteBinding (NewRemoteRef16 _)) (NewReply r) = Just $ RemoteRefW16 $ Ref16 $ fromIntegral r
parseQueryResult (RemoteBinding (NewRemoteRef32 _)) (NewReply r) = Just $ RemoteRefW32 $ Ref32 $ fromIntegral r
