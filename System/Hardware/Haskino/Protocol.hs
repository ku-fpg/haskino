...
packageCodeBlock (Arduino commands) = do
  ...
  where
      ...
      packProcedure (IterateUnitUnitE br iv bf) = do
          i <- packIterateProcedure (IterateUnitUnitE br iv bf)
          return $ RemBindUnit i
      packProcedure (IterateUnitBoolE br iv bf) = do
          i <- packIterateProcedure (IterateUnitBoolE br iv bf)
          return $ RemBindB i
      packProcedure (IterateUnitW8E br iv bf) = do
          i <- packIterateProcedure (IterateUnitW8E br iv bf)
          return $ RemBindW8 i
      packProcedure (IterateUnitW16E br iv bf) = do
          i <- packIterateProcedure (IterateUnitW16E br iv bf)
          return $ RemBindW16 i
      packProcedure (IterateUnitW32E br iv bf) = do
          i <- packIterateProcedure (IterateUnitW32E br iv bf)
          return $ RemBindW32 i
      packProcedure (IterateUnitI8E br iv bf) = do
          i <- packIterateProcedure (IterateUnitI8E br iv bf)
          return $ RemBindI8 i
      packProcedure (IterateUnitI16E br iv bf) = do
          i <- packIterateProcedure (IterateUnitI16E br iv bf)
          return $ RemBindI16 i
      packProcedure (IterateUnitI32E br iv bf) = do
          i <- packIterateProcedure (IterateUnitI32E br iv bf)
          return $ RemBindI32 i
      packProcedure (IterateUnitIE br iv bf) = do
          i <- packIterateProcedure (IterateUnitIE br iv bf)
          return $ RemBindI i
      packProcedure (IterateUnitL8E br iv bf) = do
          i <- packIterateProcedure (IterateUnitL8E br iv bf)
          return $ RemBindList8 i
      packProcedure (IterateUnitFloatE br iv bf) = do
          i <- packIterateProcedure (IterateUnitFloatE br iv bf)
          return $ RemBindFloat i
      packProcedure (IterateBoolUnitE br iv bf) = do
          i <- packIterateProcedure (IterateBoolUnitE br iv bf)
          return $ RemBindUnit i
      packProcedure (IterateBoolBoolE br iv bf) = do
          i <- packIterateProcedure (IterateBoolBoolE br iv bf)
          return $ RemBindB i
      packProcedure (IterateBoolW8E br iv bf) = do
          i <- packIterateProcedure (IterateBoolW8E br iv bf)
          return $ RemBindW8 i
      packProcedure (IterateBoolW16E br iv bf) = do
          i <- packIterateProcedure (IterateBoolW16E br iv bf)
          return $ RemBindW16 i
      packProcedure (IterateBoolW32E br iv bf) = do
          i <- packIterateProcedure (IterateBoolW32E br iv bf)
          return $ RemBindW32 i
      packProcedure (IterateBoolI8E br iv bf) = do
          i <- packIterateProcedure (IterateBoolI8E br iv bf)
          return $ RemBindI8 i
      packProcedure (IterateBoolI16E br iv bf) = do
          i <- packIterateProcedure (IterateBoolI16E br iv bf)
          return $ RemBindI16 i
      packProcedure (IterateBoolI32E br iv bf) = do
          i <- packIterateProcedure (IterateBoolI32E br iv bf)
          return $ RemBindI32 i
      packProcedure (IterateBoolIE br iv bf) = do
          i <- packIterateProcedure (IterateBoolIE br iv bf)
          return $ RemBindI i
      packProcedure (IterateBoolL8E br iv bf) = do
          i <- packIterateProcedure (IterateBoolL8E br iv bf)
          return $ RemBindList8 i
      packProcedure (IterateBoolFloatE br iv bf) = do
          i <- packIterateProcedure (IterateBoolFloatE br iv bf)
          return $ RemBindFloat i
      packProcedure (IterateW8UnitE br iv bf) = do
          i <- packIterateProcedure (IterateW8UnitE br iv bf)
          return $ RemBindUnit i
      packProcedure (IterateW8BoolE br iv bf) = do
          i <- packIterateProcedure (IterateW8BoolE br iv bf)
          return $ RemBindB i
      packProcedure (IterateW8W8E br iv bf) = do
          i <- packIterateProcedure (IterateW8W8E br iv bf)
          return $ RemBindW8 i
      packProcedure (IterateW8W16E br iv bf) = do
          i <- packIterateProcedure (IterateW8W16E br iv bf)
          return $ RemBindW16 i
      packProcedure (IterateW8W32E br iv bf) = do
          i <- packIterateProcedure (IterateW8W32E br iv bf)
          return $ RemBindW32 i
      packProcedure (IterateW8I8E br iv bf) = do
          i <- packIterateProcedure (IterateW8I8E br iv bf)
          return $ RemBindI8 i
      packProcedure (IterateW8I16E br iv bf) = do
          i <- packIterateProcedure (IterateW8I16E br iv bf)
          return $ RemBindI16 i
      packProcedure (IterateW8I32E br iv bf) = do
          i <- packIterateProcedure (IterateW8I32E br iv bf)
          return $ RemBindI32 i
      packProcedure (IterateW8IE br iv bf) = do
          i <- packIterateProcedure (IterateW8IE br iv bf)
          return $ RemBindI i
      packProcedure (IterateW8L8E br iv bf) = do
          i <- packIterateProcedure (IterateW8L8E br iv bf)
          return $ RemBindList8 i
      packProcedure (IterateW8FloatE br iv bf) = do
          i <- packIterateProcedure (IterateW8FloatE br iv bf)
          return $ RemBindFloat i
      packProcedure (IterateW16UnitE br iv bf) = do
          i <- packIterateProcedure (IterateW16UnitE br iv bf)
          return $ RemBindUnit i
      packProcedure (IterateW16BoolE br iv bf) = do
          i <- packIterateProcedure (IterateW16BoolE br iv bf)
          return $ RemBindB i
      packProcedure (IterateW16W8E br iv bf) = do
          i <- packIterateProcedure (IterateW16W8E br iv bf)
          return $ RemBindW8 i
      packProcedure (IterateW16W16E br iv bf) = do
          i <- packIterateProcedure (IterateW16W16E br iv bf)
          return $ RemBindW16 i
      packProcedure (IterateW16W32E br iv bf) = do
          i <- packIterateProcedure (IterateW16W32E br iv bf)
          return $ RemBindW32 i
      packProcedure (IterateW16I8E br iv bf) = do
          i <- packIterateProcedure (IterateW16I8E br iv bf)
          return $ RemBindI8 i
      packProcedure (IterateW16I16E br iv bf) = do
          i <- packIterateProcedure (IterateW16I16E br iv bf)
          return $ RemBindI16 i
      packProcedure (IterateW16I32E br iv bf) = do
          i <- packIterateProcedure (IterateW16I32E br iv bf)
          return $ RemBindI32 i
      packProcedure (IterateW16IE br iv bf) = do
          i <- packIterateProcedure (IterateW16IE br iv bf)
          return $ RemBindI i
      packProcedure (IterateW16L8E br iv bf) = do
          i <- packIterateProcedure (IterateW16L8E br iv bf)
          return $ RemBindList8 i
      packProcedure (IterateW16FloatE br iv bf) = do
          i <- packIterateProcedure (IterateW16FloatE br iv bf)
          return $ RemBindFloat i
      packProcedure (IterateW32UnitE br iv bf) = do
          i <- packIterateProcedure (IterateW32UnitE br iv bf)
          return $ RemBindUnit i
      packProcedure (IterateW32BoolE br iv bf) = do
          i <- packIterateProcedure (IterateW32BoolE br iv bf)
          return $ RemBindB i
      packProcedure (IterateW32W8E br iv bf) = do
          i <- packIterateProcedure (IterateW32W8E br iv bf)
          return $ RemBindW8 i
      packProcedure (IterateW32W16E br iv bf) = do
          i <- packIterateProcedure (IterateW32W16E br iv bf)
          return $ RemBindW16 i
      packProcedure (IterateW32W32E br iv bf) = do
          i <- packIterateProcedure (IterateW32W32E br iv bf)
          return $ RemBindW32 i
      packProcedure (IterateW32I8E br iv bf) = do
          i <- packIterateProcedure (IterateW32I8E br iv bf)
          return $ RemBindI8 i
      packProcedure (IterateW32I16E br iv bf) = do
          i <- packIterateProcedure (IterateW32I16E br iv bf)
          return $ RemBindI16 i
      packProcedure (IterateW32I32E br iv bf) = do
          i <- packIterateProcedure (IterateW32I32E br iv bf)
          return $ RemBindI32 i
      packProcedure (IterateW32IE br iv bf) = do
          i <- packIterateProcedure (IterateW32IE br iv bf)
          return $ RemBindI i
      packProcedure (IterateW32L8E br iv bf) = do
          i <- packIterateProcedure (IterateW32L8E br iv bf)
          return $ RemBindList8 i
      packProcedure (IterateW32FloatE br iv bf) = do
          i <- packIterateProcedure (IterateW32FloatE br iv bf)
          return $ RemBindFloat i
      packProcedure (IterateI8UnitE br iv bf) = do
          i <- packIterateProcedure (IterateI8UnitE br iv bf)
          return $ RemBindUnit i
      packProcedure (IterateI8BoolE br iv bf) = do
          i <- packIterateProcedure (IterateI8BoolE br iv bf)
          return $ RemBindB i
      packProcedure (IterateI8W8E br iv bf) = do
          i <- packIterateProcedure (IterateI8W8E br iv bf)
          return $ RemBindW8 i
      packProcedure (IterateI8W16E br iv bf) = do
          i <- packIterateProcedure (IterateI8W16E br iv bf)
          return $ RemBindW16 i
      packProcedure (IterateI8W32E br iv bf) = do
          i <- packIterateProcedure (IterateI8W32E br iv bf)
          return $ RemBindW32 i
      packProcedure (IterateI8I8E br iv bf) = do
          i <- packIterateProcedure (IterateI8I8E br iv bf)
          return $ RemBindI8 i
      packProcedure (IterateI8I16E br iv bf) = do
          i <- packIterateProcedure (IterateI8I16E br iv bf)
          return $ RemBindI16 i
      packProcedure (IterateI8I32E br iv bf) = do
          i <- packIterateProcedure (IterateI8I32E br iv bf)
          return $ RemBindI32 i
      packProcedure (IterateI8IE br iv bf) = do
          i <- packIterateProcedure (IterateI8IE br iv bf)
          return $ RemBindI i
      packProcedure (IterateI8L8E br iv bf) = do
          i <- packIterateProcedure (IterateI8L8E br iv bf)
          return $ RemBindList8 i
      packProcedure (IterateI8FloatE br iv bf) = do
          i <- packIterateProcedure (IterateI8FloatE br iv bf)
          return $ RemBindFloat i
      packProcedure (IterateI16UnitE br iv bf) = do
          i <- packIterateProcedure (IterateI16UnitE br iv bf)
          return $ RemBindUnit i
      packProcedure (IterateI16BoolE br iv bf) = do
          i <- packIterateProcedure (IterateI16BoolE br iv bf)
          return $ RemBindB i
      packProcedure (IterateI16W8E br iv bf) = do
          i <- packIterateProcedure (IterateI16W8E br iv bf)
          return $ RemBindW8 i
      packProcedure (IterateI16W16E br iv bf) = do
          i <- packIterateProcedure (IterateI16W16E br iv bf)
          return $ RemBindW16 i
      packProcedure (IterateI16W32E br iv bf) = do
          i <- packIterateProcedure (IterateI16W32E br iv bf)
          return $ RemBindW32 i
      packProcedure (IterateI16I8E br iv bf) = do
          i <- packIterateProcedure (IterateI16I8E br iv bf)
          return $ RemBindI8 i
      packProcedure (IterateI16I16E br iv bf) = do
          i <- packIterateProcedure (IterateI16I16E br iv bf)
          return $ RemBindI16 i
      packProcedure (IterateI16I32E br iv bf) = do
          i <- packIterateProcedure (IterateI16I32E br iv bf)
          return $ RemBindI32 i
      packProcedure (IterateI16IE br iv bf) = do
          i <- packIterateProcedure (IterateI16IE br iv bf)
          return $ RemBindI i
      packProcedure (IterateI16L8E br iv bf) = do
          i <- packIterateProcedure (IterateI16L8E br iv bf)
          return $ RemBindList8 i
      packProcedure (IterateI16FloatE br iv bf) = do
          i <- packIterateProcedure (IterateI16FloatE br iv bf)
          return $ RemBindFloat i
      packProcedure (IterateI32UnitE br iv bf) = do
          i <- packIterateProcedure (IterateI32UnitE br iv bf)
          return $ RemBindUnit i
      packProcedure (IterateI32BoolE br iv bf) = do
          i <- packIterateProcedure (IterateI32BoolE br iv bf)
          return $ RemBindB i
      packProcedure (IterateI32W8E br iv bf) = do
          i <- packIterateProcedure (IterateI32W8E br iv bf)
          return $ RemBindW8 i
      packProcedure (IterateI32W16E br iv bf) = do
          i <- packIterateProcedure (IterateI32W16E br iv bf)
          return $ RemBindW16 i
      packProcedure (IterateI32W32E br iv bf) = do
          i <- packIterateProcedure (IterateI32W32E br iv bf)
          return $ RemBindW32 i
      packProcedure (IterateI32I8E br iv bf) = do
          i <- packIterateProcedure (IterateI32I8E br iv bf)
          return $ RemBindI8 i
      packProcedure (IterateI32I16E br iv bf) = do
          i <- packIterateProcedure (IterateI32I16E br iv bf)
          return $ RemBindI16 i
      packProcedure (IterateI32I32E br iv bf) = do
          i <- packIterateProcedure (IterateI32I32E br iv bf)
          return $ RemBindI32 i
      packProcedure (IterateI32IE br iv bf) = do
          i <- packIterateProcedure (IterateI32IE br iv bf)
          return $ RemBindI i
      packProcedure (IterateI32L8E br iv bf) = do
          i <- packIterateProcedure (IterateI32L8E br iv bf)
          return $ RemBindList8 i
      packProcedure (IterateI32FloatE br iv bf) = do
          i <- packIterateProcedure (IterateI32FloatE br iv bf)
          return $ RemBindFloat i
      packProcedure (IterateIUnitE br iv bf) = do
          i <- packIterateProcedure (IterateIUnitE br iv bf)
          return $ RemBindUnit i
      packProcedure (IterateIBoolE br iv bf) = do
          i <- packIterateProcedure (IterateIBoolE br iv bf)
          return $ RemBindB i
      packProcedure (IterateIW8E br iv bf) = do
          i <- packIterateProcedure (IterateIW8E br iv bf)
          return $ RemBindW8 i
      packProcedure (IterateIW16E br iv bf) = do
          i <- packIterateProcedure (IterateIW16E br iv bf)
          return $ RemBindW16 i
      packProcedure (IterateIW32E br iv bf) = do
          i <- packIterateProcedure (IterateIW32E br iv bf)
          return $ RemBindW32 i
      packProcedure (IterateII8E br iv bf) = do
          i <- packIterateProcedure (IterateII8E br iv bf)
          return $ RemBindI8 i
      packProcedure (IterateII16E br iv bf) = do
          i <- packIterateProcedure (IterateII16E br iv bf)
          return $ RemBindI16 i
      packProcedure (IterateII32E br iv bf) = do
          i <- packIterateProcedure (IterateII32E br iv bf)
          return $ RemBindI32 i
      packProcedure (IterateIIE br iv bf) = do
          i <- packIterateProcedure (IterateIIE br iv bf)
          return $ RemBindI i
      packProcedure (IterateIL8E br iv bf) = do
          i <- packIterateProcedure (IterateIL8E br iv bf)
          return $ RemBindList8 i
      packProcedure (IterateIFloatE br iv bf) = do
          i <- packIterateProcedure (IterateIFloatE br iv bf)
          return $ RemBindFloat i
      packProcedure (IterateL8UnitE br iv bf) = do
          i <- packIterateProcedure (IterateL8UnitE br iv bf)
          return $ RemBindUnit i
      packProcedure (IterateL8BoolE br iv bf) = do
          i <- packIterateProcedure (IterateL8BoolE br iv bf)
          return $ RemBindB i
      packProcedure (IterateL8W8E br iv bf) = do
          i <- packIterateProcedure (IterateL8W8E br iv bf)
          return $ RemBindW8 i
      packProcedure (IterateL8W16E br iv bf) = do
          i <- packIterateProcedure (IterateL8W16E br iv bf)
          return $ RemBindW16 i
      packProcedure (IterateL8W32E br iv bf) = do
          i <- packIterateProcedure (IterateL8W32E br iv bf)
          return $ RemBindW32 i
      packProcedure (IterateL8I8E br iv bf) = do
          i <- packIterateProcedure (IterateL8I8E br iv bf)
          return $ RemBindI8 i
      packProcedure (IterateL8I16E br iv bf) = do
          i <- packIterateProcedure (IterateL8I16E br iv bf)
          return $ RemBindI16 i
      packProcedure (IterateL8I32E br iv bf) = do
          i <- packIterateProcedure (IterateL8I32E br iv bf)
          return $ RemBindI32 i
      packProcedure (IterateL8IE br iv bf) = do
          i <- packIterateProcedure (IterateL8IE br iv bf)
          return $ RemBindI i
      packProcedure (IterateL8L8E br iv bf) = do
          i <- packIterateProcedure (IterateL8L8E br iv bf)
          return $ RemBindList8 i
      packProcedure (IterateL8FloatE br iv bf) = do
          i <- packIterateProcedure (IterateL8FloatE br iv bf)
          return $ RemBindFloat i
      packProcedure (IterateFloatUnitE br iv bf) = do
          i <- packIterateProcedure (IterateFloatUnitE br iv bf)
          return $ RemBindUnit i
      packProcedure (IterateFloatBoolE br iv bf) = do
          i <- packIterateProcedure (IterateFloatBoolE br iv bf)
          return $ RemBindB i
      packProcedure (IterateFloatW8E br iv bf) = do
          i <- packIterateProcedure (IterateFloatW8E br iv bf)
          return $ RemBindW8 i
      packProcedure (IterateFloatW16E br iv bf) = do
          i <- packIterateProcedure (IterateFloatW16E br iv bf)
          return $ RemBindW16 i
      packProcedure (IterateFloatW32E br iv bf) = do
          i <- packIterateProcedure (IterateFloatW32E br iv bf)
          return $ RemBindW32 i
      packProcedure (IterateFloatI8E br iv bf) = do
          i <- packIterateProcedure (IterateFloatI8E br iv bf)
          return $ RemBindI8 i
      packProcedure (IterateFloatI16E br iv bf) = do
          i <- packIterateProcedure (IterateFloatI16E br iv bf)
          return $ RemBindI16 i
      packProcedure (IterateFloatI32E br iv bf) = do
          i <- packIterateProcedure (IterateFloatI32E br iv bf)
          return $ RemBindI32 i
      packProcedure (IterateFloatIE br iv bf) = do
          i <- packIterateProcedure (IterateFloatIE br iv bf)
          return $ RemBindI i
      packProcedure (IterateFloatL8E br iv bf) = do
          i <- packIterateProcedure (IterateFloatL8E br iv bf)
          return $ RemBindList8 i
      packProcedure (IterateFloatFloatE br iv bf) = do
          i <- packIterateProcedure (IterateFloatFloatE br iv bf)
          return $ RemBindFloat i
...
packageProcedure p = do
  ...
  where
    ...
    packageProcedure' (IfThenElseFloatFloat e cb1 cb2) ib' = packageIfThenElseEitherProcedure EXPR_FLOAT EXPR_FLOAT ib' e cb1 cb2
      -- The following Iterate*E functions generated by toold/GenEitherTypes.hs
    packageProcedure' (IterateUnitUnitE br iv bf) ib' = packageIterateProcedure br EXPR_UNIT EXPR_UNIT ib' (RemBindUnit ib') iv bf
    packageProcedure' (IterateUnitBoolE br iv bf) ib' = packageIterateProcedure br EXPR_UNIT EXPR_BOOL ib' (RemBindUnit ib') iv bf
    packageProcedure' (IterateUnitW8E br iv bf) ib' = packageIterateProcedure br EXPR_UNIT EXPR_WORD8 ib' (RemBindUnit ib') iv bf
    packageProcedure' (IterateUnitW16E br iv bf) ib' = packageIterateProcedure br EXPR_UNIT EXPR_WORD16 ib' (RemBindUnit ib') iv bf
    packageProcedure' (IterateUnitW32E br iv bf) ib' = packageIterateProcedure br EXPR_UNIT EXPR_WORD32 ib' (RemBindUnit ib') iv bf
    packageProcedure' (IterateUnitI8E br iv bf) ib' = packageIterateProcedure br EXPR_UNIT EXPR_INT8 ib' (RemBindUnit ib') iv bf
    packageProcedure' (IterateUnitI16E br iv bf) ib' = packageIterateProcedure br EXPR_UNIT EXPR_INT16 ib' (RemBindUnit ib') iv bf
    packageProcedure' (IterateUnitI32E br iv bf) ib' = packageIterateProcedure br EXPR_UNIT EXPR_INT32 ib' (RemBindUnit ib') iv bf
    packageProcedure' (IterateUnitIE br iv bf) ib' = packageIterateProcedure br EXPR_UNIT EXPR_INT32 ib' (RemBindUnit ib') iv bf
    packageProcedure' (IterateUnitL8E br iv bf) ib' = packageIterateProcedure br EXPR_UNIT EXPR_LIST8 ib' (RemBindUnit ib') iv bf
    packageProcedure' (IterateUnitFloatE br iv bf) ib' = packageIterateProcedure br EXPR_UNIT EXPR_FLOAT ib' (RemBindUnit ib') iv bf
    packageProcedure' (IterateBoolUnitE br iv bf) ib' = packageIterateProcedure br EXPR_BOOL EXPR_UNIT ib' (RemBindB ib') iv bf
    packageProcedure' (IterateBoolBoolE br iv bf) ib' = packageIterateProcedure br EXPR_BOOL EXPR_BOOL ib' (RemBindB ib') iv bf
    packageProcedure' (IterateBoolW8E br iv bf) ib' = packageIterateProcedure br EXPR_BOOL EXPR_WORD8 ib' (RemBindB ib') iv bf
    packageProcedure' (IterateBoolW16E br iv bf) ib' = packageIterateProcedure br EXPR_BOOL EXPR_WORD16 ib' (RemBindB ib') iv bf
    packageProcedure' (IterateBoolW32E br iv bf) ib' = packageIterateProcedure br EXPR_BOOL EXPR_WORD32 ib' (RemBindB ib') iv bf
    packageProcedure' (IterateBoolI8E br iv bf) ib' = packageIterateProcedure br EXPR_BOOL EXPR_INT8 ib' (RemBindB ib') iv bf
    packageProcedure' (IterateBoolI16E br iv bf) ib' = packageIterateProcedure br EXPR_BOOL EXPR_INT16 ib' (RemBindB ib') iv bf
    packageProcedure' (IterateBoolI32E br iv bf) ib' = packageIterateProcedure br EXPR_BOOL EXPR_INT32 ib' (RemBindB ib') iv bf
    packageProcedure' (IterateBoolIE br iv bf) ib' = packageIterateProcedure br EXPR_BOOL EXPR_INT32 ib' (RemBindB ib') iv bf
    packageProcedure' (IterateBoolL8E br iv bf) ib' = packageIterateProcedure br EXPR_BOOL EXPR_LIST8 ib' (RemBindB ib') iv bf
    packageProcedure' (IterateBoolFloatE br iv bf) ib' = packageIterateProcedure br EXPR_BOOL EXPR_FLOAT ib' (RemBindB ib') iv bf
    packageProcedure' (IterateW8UnitE br iv bf) ib' = packageIterateProcedure br EXPR_WORD8 EXPR_UNIT ib' (RemBindW8 ib') iv bf
    packageProcedure' (IterateW8BoolE br iv bf) ib' = packageIterateProcedure br EXPR_WORD8 EXPR_BOOL ib' (RemBindW8 ib') iv bf
    packageProcedure' (IterateW8W8E br iv bf) ib' = packageIterateProcedure br EXPR_WORD8 EXPR_WORD8 ib' (RemBindW8 ib') iv bf
    packageProcedure' (IterateW8W16E br iv bf) ib' = packageIterateProcedure br EXPR_WORD8 EXPR_WORD16 ib' (RemBindW8 ib') iv bf
    packageProcedure' (IterateW8W32E br iv bf) ib' = packageIterateProcedure br EXPR_WORD8 EXPR_WORD32 ib' (RemBindW8 ib') iv bf
    packageProcedure' (IterateW8I8E br iv bf) ib' = packageIterateProcedure br EXPR_WORD8 EXPR_INT8 ib' (RemBindW8 ib') iv bf
    packageProcedure' (IterateW8I16E br iv bf) ib' = packageIterateProcedure br EXPR_WORD8 EXPR_INT16 ib' (RemBindW8 ib') iv bf
    packageProcedure' (IterateW8I32E br iv bf) ib' = packageIterateProcedure br EXPR_WORD8 EXPR_INT32 ib' (RemBindW8 ib') iv bf
    packageProcedure' (IterateW8IE br iv bf) ib' = packageIterateProcedure br EXPR_WORD8 EXPR_INT32 ib' (RemBindW8 ib') iv bf
    packageProcedure' (IterateW8L8E br iv bf) ib' = packageIterateProcedure br EXPR_WORD8 EXPR_LIST8 ib' (RemBindW8 ib') iv bf
    packageProcedure' (IterateW8FloatE br iv bf) ib' = packageIterateProcedure br EXPR_WORD8 EXPR_FLOAT ib' (RemBindW8 ib') iv bf
    packageProcedure' (IterateW16UnitE br iv bf) ib' = packageIterateProcedure br EXPR_WORD16 EXPR_UNIT ib' (RemBindW16 ib') iv bf
    packageProcedure' (IterateW16BoolE br iv bf) ib' = packageIterateProcedure br EXPR_WORD16 EXPR_BOOL ib' (RemBindW16 ib') iv bf
    packageProcedure' (IterateW16W8E br iv bf) ib' = packageIterateProcedure br EXPR_WORD16 EXPR_WORD8 ib' (RemBindW16 ib') iv bf
    packageProcedure' (IterateW16W16E br iv bf) ib' = packageIterateProcedure br EXPR_WORD16 EXPR_WORD16 ib' (RemBindW16 ib') iv bf
    packageProcedure' (IterateW16W32E br iv bf) ib' = packageIterateProcedure br EXPR_WORD16 EXPR_WORD32 ib' (RemBindW16 ib') iv bf
    packageProcedure' (IterateW16I8E br iv bf) ib' = packageIterateProcedure br EXPR_WORD16 EXPR_INT8 ib' (RemBindW16 ib') iv bf
    packageProcedure' (IterateW16I16E br iv bf) ib' = packageIterateProcedure br EXPR_WORD16 EXPR_INT16 ib' (RemBindW16 ib') iv bf
    packageProcedure' (IterateW16I32E br iv bf) ib' = packageIterateProcedure br EXPR_WORD16 EXPR_INT32 ib' (RemBindW16 ib') iv bf
    packageProcedure' (IterateW16IE br iv bf) ib' = packageIterateProcedure br EXPR_WORD16 EXPR_INT32 ib' (RemBindW16 ib') iv bf
    packageProcedure' (IterateW16L8E br iv bf) ib' = packageIterateProcedure br EXPR_WORD16 EXPR_LIST8 ib' (RemBindW16 ib') iv bf
    packageProcedure' (IterateW16FloatE br iv bf) ib' = packageIterateProcedure br EXPR_WORD16 EXPR_FLOAT ib' (RemBindW16 ib') iv bf
    packageProcedure' (IterateW32UnitE br iv bf) ib' = packageIterateProcedure br EXPR_WORD32 EXPR_UNIT ib' (RemBindW32 ib') iv bf
    packageProcedure' (IterateW32BoolE br iv bf) ib' = packageIterateProcedure br EXPR_WORD32 EXPR_BOOL ib' (RemBindW32 ib') iv bf
    packageProcedure' (IterateW32W8E br iv bf) ib' = packageIterateProcedure br EXPR_WORD32 EXPR_WORD8 ib' (RemBindW32 ib') iv bf
    packageProcedure' (IterateW32W16E br iv bf) ib' = packageIterateProcedure br EXPR_WORD32 EXPR_WORD16 ib' (RemBindW32 ib') iv bf
    packageProcedure' (IterateW32W32E br iv bf) ib' = packageIterateProcedure br EXPR_WORD32 EXPR_WORD32 ib' (RemBindW32 ib') iv bf
    packageProcedure' (IterateW32I8E br iv bf) ib' = packageIterateProcedure br EXPR_WORD32 EXPR_INT8 ib' (RemBindW32 ib') iv bf
    packageProcedure' (IterateW32I16E br iv bf) ib' = packageIterateProcedure br EXPR_WORD32 EXPR_INT16 ib' (RemBindW32 ib') iv bf
    packageProcedure' (IterateW32I32E br iv bf) ib' = packageIterateProcedure br EXPR_WORD32 EXPR_INT32 ib' (RemBindW32 ib') iv bf
    packageProcedure' (IterateW32IE br iv bf) ib' = packageIterateProcedure br EXPR_WORD32 EXPR_INT32 ib' (RemBindW32 ib') iv bf
    packageProcedure' (IterateW32L8E br iv bf) ib' = packageIterateProcedure br EXPR_WORD32 EXPR_LIST8 ib' (RemBindW32 ib') iv bf
    packageProcedure' (IterateW32FloatE br iv bf) ib' = packageIterateProcedure br EXPR_WORD32 EXPR_FLOAT ib' (RemBindW32 ib') iv bf
    packageProcedure' (IterateI8UnitE br iv bf) ib' = packageIterateProcedure br EXPR_INT8 EXPR_UNIT ib' (RemBindI8 ib') iv bf
    packageProcedure' (IterateI8BoolE br iv bf) ib' = packageIterateProcedure br EXPR_INT8 EXPR_BOOL ib' (RemBindI8 ib') iv bf
    packageProcedure' (IterateI8W8E br iv bf) ib' = packageIterateProcedure br EXPR_INT8 EXPR_WORD8 ib' (RemBindI8 ib') iv bf
    packageProcedure' (IterateI8W16E br iv bf) ib' = packageIterateProcedure br EXPR_INT8 EXPR_WORD16 ib' (RemBindI8 ib') iv bf
    packageProcedure' (IterateI8W32E br iv bf) ib' = packageIterateProcedure br EXPR_INT8 EXPR_WORD32 ib' (RemBindI8 ib') iv bf
    packageProcedure' (IterateI8I8E br iv bf) ib' = packageIterateProcedure br EXPR_INT8 EXPR_INT8 ib' (RemBindI8 ib') iv bf
    packageProcedure' (IterateI8I16E br iv bf) ib' = packageIterateProcedure br EXPR_INT8 EXPR_INT16 ib' (RemBindI8 ib') iv bf
    packageProcedure' (IterateI8I32E br iv bf) ib' = packageIterateProcedure br EXPR_INT8 EXPR_INT32 ib' (RemBindI8 ib') iv bf
    packageProcedure' (IterateI8IE br iv bf) ib' = packageIterateProcedure br EXPR_INT8 EXPR_INT32 ib' (RemBindI8 ib') iv bf
    packageProcedure' (IterateI8L8E br iv bf) ib' = packageIterateProcedure br EXPR_INT8 EXPR_LIST8 ib' (RemBindI8 ib') iv bf
    packageProcedure' (IterateI8FloatE br iv bf) ib' = packageIterateProcedure br EXPR_INT8 EXPR_FLOAT ib' (RemBindI8 ib') iv bf
    packageProcedure' (IterateI16UnitE br iv bf) ib' = packageIterateProcedure br EXPR_INT16 EXPR_UNIT ib' (RemBindI16 ib') iv bf
    packageProcedure' (IterateI16BoolE br iv bf) ib' = packageIterateProcedure br EXPR_INT16 EXPR_BOOL ib' (RemBindI16 ib') iv bf
    packageProcedure' (IterateI16W8E br iv bf) ib' = packageIterateProcedure br EXPR_INT16 EXPR_WORD8 ib' (RemBindI16 ib') iv bf
    packageProcedure' (IterateI16W16E br iv bf) ib' = packageIterateProcedure br EXPR_INT16 EXPR_WORD16 ib' (RemBindI16 ib') iv bf
    packageProcedure' (IterateI16W32E br iv bf) ib' = packageIterateProcedure br EXPR_INT16 EXPR_WORD32 ib' (RemBindI16 ib') iv bf
    packageProcedure' (IterateI16I8E br iv bf) ib' = packageIterateProcedure br EXPR_INT16 EXPR_INT8 ib' (RemBindI16 ib') iv bf
    packageProcedure' (IterateI16I16E br iv bf) ib' = packageIterateProcedure br EXPR_INT16 EXPR_INT16 ib' (RemBindI16 ib') iv bf
    packageProcedure' (IterateI16I32E br iv bf) ib' = packageIterateProcedure br EXPR_INT16 EXPR_INT32 ib' (RemBindI16 ib') iv bf
    packageProcedure' (IterateI16IE br iv bf) ib' = packageIterateProcedure br EXPR_INT16 EXPR_INT32 ib' (RemBindI16 ib') iv bf
    packageProcedure' (IterateI16L8E br iv bf) ib' = packageIterateProcedure br EXPR_INT16 EXPR_LIST8 ib' (RemBindI16 ib') iv bf
    packageProcedure' (IterateI16FloatE br iv bf) ib' = packageIterateProcedure br EXPR_INT16 EXPR_FLOAT ib' (RemBindI16 ib') iv bf
    packageProcedure' (IterateI32UnitE br iv bf) ib' = packageIterateProcedure br EXPR_INT32 EXPR_UNIT ib' (RemBindI32 ib') iv bf
    packageProcedure' (IterateI32BoolE br iv bf) ib' = packageIterateProcedure br EXPR_INT32 EXPR_BOOL ib' (RemBindI32 ib') iv bf
    packageProcedure' (IterateI32W8E br iv bf) ib' = packageIterateProcedure br EXPR_INT32 EXPR_WORD8 ib' (RemBindI32 ib') iv bf
    packageProcedure' (IterateI32W16E br iv bf) ib' = packageIterateProcedure br EXPR_INT32 EXPR_WORD16 ib' (RemBindI32 ib') iv bf
    packageProcedure' (IterateI32W32E br iv bf) ib' = packageIterateProcedure br EXPR_INT32 EXPR_WORD32 ib' (RemBindI32 ib') iv bf
    packageProcedure' (IterateI32I8E br iv bf) ib' = packageIterateProcedure br EXPR_INT32 EXPR_INT8 ib' (RemBindI32 ib') iv bf
    packageProcedure' (IterateI32I16E br iv bf) ib' = packageIterateProcedure br EXPR_INT32 EXPR_INT16 ib' (RemBindI32 ib') iv bf
    packageProcedure' (IterateI32I32E br iv bf) ib' = packageIterateProcedure br EXPR_INT32 EXPR_INT32 ib' (RemBindI32 ib') iv bf
    packageProcedure' (IterateI32IE br iv bf) ib' = packageIterateProcedure br EXPR_INT32 EXPR_INT32 ib' (RemBindI32 ib') iv bf
    packageProcedure' (IterateI32L8E br iv bf) ib' = packageIterateProcedure br EXPR_INT32 EXPR_LIST8 ib' (RemBindI32 ib') iv bf
    packageProcedure' (IterateI32FloatE br iv bf) ib' = packageIterateProcedure br EXPR_INT32 EXPR_FLOAT ib' (RemBindI32 ib') iv bf
    packageProcedure' (IterateIUnitE br iv bf) ib' = packageIterateProcedure br EXPR_INT32 EXPR_UNIT ib' (RemBindI ib') iv bf
    packageProcedure' (IterateIBoolE br iv bf) ib' = packageIterateProcedure br EXPR_INT32 EXPR_BOOL ib' (RemBindI ib') iv bf
    packageProcedure' (IterateIW8E br iv bf) ib' = packageIterateProcedure br EXPR_INT32 EXPR_WORD8 ib' (RemBindI ib') iv bf
    packageProcedure' (IterateIW16E br iv bf) ib' = packageIterateProcedure br EXPR_INT32 EXPR_WORD16 ib' (RemBindI ib') iv bf
    packageProcedure' (IterateIW32E br iv bf) ib' = packageIterateProcedure br EXPR_INT32 EXPR_WORD32 ib' (RemBindI ib') iv bf
    packageProcedure' (IterateII8E br iv bf) ib' = packageIterateProcedure br EXPR_INT32 EXPR_INT8 ib' (RemBindI ib') iv bf
    packageProcedure' (IterateII16E br iv bf) ib' = packageIterateProcedure br EXPR_INT32 EXPR_INT16 ib' (RemBindI ib') iv bf
    packageProcedure' (IterateII32E br iv bf) ib' = packageIterateProcedure br EXPR_INT32 EXPR_INT32 ib' (RemBindI ib') iv bf
    packageProcedure' (IterateIIE br iv bf) ib' = packageIterateProcedure br EXPR_INT32 EXPR_INT32 ib' (RemBindI ib') iv bf
    packageProcedure' (IterateIL8E br iv bf) ib' = packageIterateProcedure br EXPR_INT32 EXPR_LIST8 ib' (RemBindI ib') iv bf
    packageProcedure' (IterateIFloatE br iv bf) ib' = packageIterateProcedure br EXPR_INT32 EXPR_FLOAT ib' (RemBindI ib') iv bf
    packageProcedure' (IterateL8UnitE br iv bf) ib' = packageIterateProcedure br EXPR_LIST8 EXPR_UNIT ib' (RemBindList8 ib') iv bf
    packageProcedure' (IterateL8BoolE br iv bf) ib' = packageIterateProcedure br EXPR_LIST8 EXPR_BOOL ib' (RemBindList8 ib') iv bf
    packageProcedure' (IterateL8W8E br iv bf) ib' = packageIterateProcedure br EXPR_LIST8 EXPR_WORD8 ib' (RemBindList8 ib') iv bf
    packageProcedure' (IterateL8W16E br iv bf) ib' = packageIterateProcedure br EXPR_LIST8 EXPR_WORD16 ib' (RemBindList8 ib') iv bf
    packageProcedure' (IterateL8W32E br iv bf) ib' = packageIterateProcedure br EXPR_LIST8 EXPR_WORD32 ib' (RemBindList8 ib') iv bf
    packageProcedure' (IterateL8I8E br iv bf) ib' = packageIterateProcedure br EXPR_LIST8 EXPR_INT8 ib' (RemBindList8 ib') iv bf
    packageProcedure' (IterateL8I16E br iv bf) ib' = packageIterateProcedure br EXPR_LIST8 EXPR_INT16 ib' (RemBindList8 ib') iv bf
    packageProcedure' (IterateL8I32E br iv bf) ib' = packageIterateProcedure br EXPR_LIST8 EXPR_INT32 ib' (RemBindList8 ib') iv bf
    packageProcedure' (IterateL8IE br iv bf) ib' = packageIterateProcedure br EXPR_LIST8 EXPR_INT32 ib' (RemBindList8 ib') iv bf
    packageProcedure' (IterateL8L8E br iv bf) ib' = packageIterateProcedure br EXPR_LIST8 EXPR_LIST8 ib' (RemBindList8 ib') iv bf
    packageProcedure' (IterateL8FloatE br iv bf) ib' = packageIterateProcedure br EXPR_LIST8 EXPR_FLOAT ib' (RemBindList8 ib') iv bf
    packageProcedure' (IterateFloatUnitE br iv bf) ib' = packageIterateProcedure br EXPR_FLOAT EXPR_UNIT ib' (RemBindFloat ib') iv bf
    packageProcedure' (IterateFloatBoolE br iv bf) ib' = packageIterateProcedure br EXPR_FLOAT EXPR_BOOL ib' (RemBindFloat ib') iv bf
    packageProcedure' (IterateFloatW8E br iv bf) ib' = packageIterateProcedure br EXPR_FLOAT EXPR_WORD8 ib' (RemBindFloat ib') iv bf
    packageProcedure' (IterateFloatW16E br iv bf) ib' = packageIterateProcedure br EXPR_FLOAT EXPR_WORD16 ib' (RemBindFloat ib') iv bf
    packageProcedure' (IterateFloatW32E br iv bf) ib' = packageIterateProcedure br EXPR_FLOAT EXPR_WORD32 ib' (RemBindFloat ib') iv bf
    packageProcedure' (IterateFloatI8E br iv bf) ib' = packageIterateProcedure br EXPR_FLOAT EXPR_INT8 ib' (RemBindFloat ib') iv bf
    packageProcedure' (IterateFloatI16E br iv bf) ib' = packageIterateProcedure br EXPR_FLOAT EXPR_INT16 ib' (RemBindFloat ib') iv bf
    packageProcedure' (IterateFloatI32E br iv bf) ib' = packageIterateProcedure br EXPR_FLOAT EXPR_INT32 ib' (RemBindFloat ib') iv bf
    packageProcedure' (IterateFloatIE br iv bf) ib' = packageIterateProcedure br EXPR_FLOAT EXPR_INT32 ib' (RemBindFloat ib') iv bf
    packageProcedure' (IterateFloatL8E br iv bf) ib' = packageIterateProcedure br EXPR_FLOAT EXPR_LIST8 ib' (RemBindFloat ib') iv bf
    packageProcedure' (IterateFloatFloatE br iv bf) ib' = packageIterateProcedure br EXPR_FLOAT EXPR_FLOAT ib' (RemBindFloat ib') iv bf
...
packageIfThenElseEitherProcedure rt1 rt2 _ e cb1 cb2 = do
    s <- get
    -- TBD fix using other bind.
    let (ib1, ib2) = head $ iterBinds s
    (r1, pc1, _) <- packageCodeBlock cb1
    let rc1 = buildCommand EXPR_CMD_RET $ (fromIntegral ib2) : packageExprEither rt1 rt2 r1
    let pc1'  = B.append pc1 $ lenPackage rc1
    (r2, pc2, _) <- packageCodeBlock cb2
    let rc2 = buildCommand EXPR_CMD_RET $ (fromIntegral ib2) : packageExprEither rt1 rt2 r2
    let pc2'  = B.append pc2 $ lenPackage rc2
    let thenSize = word16ToBytes $ fromIntegral (B.length pc1')
    i <- addCommand BC_CMD_IF_THEN_ELSE ([fromIntegral $ fromEnum rt1, fromIntegral $ fromEnum rt2, fromIntegral ib2] ++ thenSize ++ (packageExpr e))
    return $ B.append i (B.append pc1' pc2')
...
-- TBD Fix ExprLeft packaging
packageExprEither t1  _t2 (ExprLeft i el) = [toW8 t1, toW8 EXPR_LEFT] ++ packageExpr el
...
-- TBD fix below with reverse op code
packageExpr (RevList8 e1) = packageSubExpr (exprLCmdVal EXPRL_LEN) e1
...
parseQueryResult (IterateUnitUnitE _ _ _) (IterateUnitReply) = Just $ LitUnit
parseQueryResult (IterateUnitBoolE _ _ _) (IterateBoolReply r) = Just $ lit r
parseQueryResult (IterateUnitW8E _ _ _) (IterateW8Reply r) = Just $ lit r
parseQueryResult (IterateUnitW16E _ _ _) (IterateW16Reply r) = Just $ lit r
parseQueryResult (IterateUnitW32E _ _ _) (IterateW32Reply r) = Just $ lit r
parseQueryResult (IterateUnitI8E _ _ _) (IterateI8Reply r) = Just $ lit r
parseQueryResult (IterateUnitI16E _ _ _) (IterateI16Reply r) = Just $ lit r
parseQueryResult (IterateUnitI32E _ _ _) (IterateI32Reply r) = Just $ lit r
parseQueryResult (IterateUnitIE _ _ _) (IterateI32Reply r) = Just $ lit (fromIntegral r)
parseQueryResult (IterateUnitL8E _ _ _) (IterateL8Reply r) = Just $ lit r
parseQueryResult (IterateUnitFloatE _ _ _) (IterateFloatReply r) = Just $ lit r
parseQueryResult (IterateBoolUnitE _ _ _) (IterateUnitReply) = Just $ LitUnit
parseQueryResult (IterateBoolBoolE _ _ _) (IterateBoolReply r) = Just $ lit r
parseQueryResult (IterateBoolW8E _ _ _) (IterateW8Reply r) = Just $ lit r
parseQueryResult (IterateBoolW16E _ _ _) (IterateW16Reply r) = Just $ lit r
parseQueryResult (IterateBoolW32E _ _ _) (IterateW32Reply r) = Just $ lit r
parseQueryResult (IterateBoolI8E _ _ _) (IterateI8Reply r) = Just $ lit r
parseQueryResult (IterateBoolI16E _ _ _) (IterateI16Reply r) = Just $ lit r
parseQueryResult (IterateBoolI32E _ _ _) (IterateI32Reply r) = Just $ lit r
parseQueryResult (IterateBoolIE _ _ _) (IterateI32Reply r) = Just $ lit (fromIntegral r)
parseQueryResult (IterateBoolL8E _ _ _) (IterateL8Reply r) = Just $ lit r
parseQueryResult (IterateBoolFloatE _ _ _) (IterateFloatReply r) = Just $ lit r
parseQueryResult (IterateW8UnitE _ _ _) (IterateUnitReply) = Just $ LitUnit
parseQueryResult (IterateW8BoolE _ _ _) (IterateBoolReply r) = Just $ lit r
parseQueryResult (IterateW8W8E _ _ _) (IterateW8Reply r) = Just $ lit r
parseQueryResult (IterateW8W16E _ _ _) (IterateW16Reply r) = Just $ lit r
parseQueryResult (IterateW8W32E _ _ _) (IterateW32Reply r) = Just $ lit r
parseQueryResult (IterateW8I8E _ _ _) (IterateI8Reply r) = Just $ lit r
parseQueryResult (IterateW8I16E _ _ _) (IterateI16Reply r) = Just $ lit r
parseQueryResult (IterateW8I32E _ _ _) (IterateI32Reply r) = Just $ lit r
parseQueryResult (IterateW8IE _ _ _) (IterateI32Reply r) = Just $ lit (fromIntegral r)
parseQueryResult (IterateW8L8E _ _ _) (IterateL8Reply r) = Just $ lit r
parseQueryResult (IterateW8FloatE _ _ _) (IterateFloatReply r) = Just $ lit r
parseQueryResult (IterateW16UnitE _ _ _) (IterateUnitReply) = Just $ LitUnit
parseQueryResult (IterateW16BoolE _ _ _) (IterateBoolReply r) = Just $ lit r
parseQueryResult (IterateW16W8E _ _ _) (IterateW8Reply r) = Just $ lit r
parseQueryResult (IterateW16W16E _ _ _) (IterateW16Reply r) = Just $ lit r
parseQueryResult (IterateW16W32E _ _ _) (IterateW32Reply r) = Just $ lit r
parseQueryResult (IterateW16I8E _ _ _) (IterateI8Reply r) = Just $ lit r
parseQueryResult (IterateW16I16E _ _ _) (IterateI16Reply r) = Just $ lit r
parseQueryResult (IterateW16I32E _ _ _) (IterateI32Reply r) = Just $ lit r
parseQueryResult (IterateW16IE _ _ _) (IterateI32Reply r) = Just $ lit (fromIntegral r)
parseQueryResult (IterateW16L8E _ _ _) (IterateL8Reply r) = Just $ lit r
parseQueryResult (IterateW16FloatE _ _ _) (IterateFloatReply r) = Just $ lit r
parseQueryResult (IterateW32UnitE _ _ _) (IterateUnitReply) = Just $ LitUnit
parseQueryResult (IterateW32BoolE _ _ _) (IterateBoolReply r) = Just $ lit r
parseQueryResult (IterateW32W8E _ _ _) (IterateW8Reply r) = Just $ lit r
parseQueryResult (IterateW32W16E _ _ _) (IterateW16Reply r) = Just $ lit r
parseQueryResult (IterateW32W32E _ _ _) (IterateW32Reply r) = Just $ lit r
parseQueryResult (IterateW32I8E _ _ _) (IterateI8Reply r) = Just $ lit r
parseQueryResult (IterateW32I16E _ _ _) (IterateI16Reply r) = Just $ lit r
parseQueryResult (IterateW32I32E _ _ _) (IterateI32Reply r) = Just $ lit r
parseQueryResult (IterateW32IE _ _ _) (IterateI32Reply r) = Just $ lit (fromIntegral r)
parseQueryResult (IterateW32L8E _ _ _) (IterateL8Reply r) = Just $ lit r
parseQueryResult (IterateW32FloatE _ _ _) (IterateFloatReply r) = Just $ lit r
parseQueryResult (IterateI8UnitE _ _ _) (IterateUnitReply) = Just $ LitUnit
parseQueryResult (IterateI8BoolE _ _ _) (IterateBoolReply r) = Just $ lit r
parseQueryResult (IterateI8W8E _ _ _) (IterateW8Reply r) = Just $ lit r
parseQueryResult (IterateI8W16E _ _ _) (IterateW16Reply r) = Just $ lit r
parseQueryResult (IterateI8W32E _ _ _) (IterateW32Reply r) = Just $ lit r
parseQueryResult (IterateI8I8E _ _ _) (IterateI8Reply r) = Just $ lit r
parseQueryResult (IterateI8I16E _ _ _) (IterateI16Reply r) = Just $ lit r
parseQueryResult (IterateI8I32E _ _ _) (IterateI32Reply r) = Just $ lit r
parseQueryResult (IterateI8IE _ _ _) (IterateI32Reply r) = Just $ lit (fromIntegral r)
parseQueryResult (IterateI8L8E _ _ _) (IterateL8Reply r) = Just $ lit r
parseQueryResult (IterateI8FloatE _ _ _) (IterateFloatReply r) = Just $ lit r
parseQueryResult (IterateI16UnitE _ _ _) (IterateUnitReply) = Just $ LitUnit
parseQueryResult (IterateI16BoolE _ _ _) (IterateBoolReply r) = Just $ lit r
parseQueryResult (IterateI16W8E _ _ _) (IterateW8Reply r) = Just $ lit r
parseQueryResult (IterateI16W16E _ _ _) (IterateW16Reply r) = Just $ lit r
parseQueryResult (IterateI16W32E _ _ _) (IterateW32Reply r) = Just $ lit r
parseQueryResult (IterateI16I8E _ _ _) (IterateI8Reply r) = Just $ lit r
parseQueryResult (IterateI16I16E _ _ _) (IterateI16Reply r) = Just $ lit r
parseQueryResult (IterateI16I32E _ _ _) (IterateI32Reply r) = Just $ lit r
parseQueryResult (IterateI16IE _ _ _) (IterateI32Reply r) = Just $ lit (fromIntegral r)
parseQueryResult (IterateI16L8E _ _ _) (IterateL8Reply r) = Just $ lit r
parseQueryResult (IterateI16FloatE _ _ _) (IterateFloatReply r) = Just $ lit r
parseQueryResult (IterateI32UnitE _ _ _) (IterateUnitReply) = Just $ LitUnit
parseQueryResult (IterateI32BoolE _ _ _) (IterateBoolReply r) = Just $ lit r
parseQueryResult (IterateI32W8E _ _ _) (IterateW8Reply r) = Just $ lit r
parseQueryResult (IterateI32W16E _ _ _) (IterateW16Reply r) = Just $ lit r
parseQueryResult (IterateI32W32E _ _ _) (IterateW32Reply r) = Just $ lit r
parseQueryResult (IterateI32I8E _ _ _) (IterateI8Reply r) = Just $ lit r
parseQueryResult (IterateI32I16E _ _ _) (IterateI16Reply r) = Just $ lit r
parseQueryResult (IterateI32I32E _ _ _) (IterateI32Reply r) = Just $ lit r
parseQueryResult (IterateI32IE _ _ _) (IterateI32Reply r) = Just $ lit (fromIntegral r)
parseQueryResult (IterateI32L8E _ _ _) (IterateL8Reply r) = Just $ lit r
parseQueryResult (IterateI32FloatE _ _ _) (IterateFloatReply r) = Just $ lit r
parseQueryResult (IterateIUnitE _ _ _) (IterateUnitReply) = Just $ LitUnit
parseQueryResult (IterateIBoolE _ _ _) (IterateBoolReply r) = Just $ lit r
parseQueryResult (IterateIW8E _ _ _) (IterateW8Reply r) = Just $ lit r
parseQueryResult (IterateIW16E _ _ _) (IterateW16Reply r) = Just $ lit r
parseQueryResult (IterateIW32E _ _ _) (IterateW32Reply r) = Just $ lit r
parseQueryResult (IterateII8E _ _ _) (IterateI8Reply r) = Just $ lit r
parseQueryResult (IterateII16E _ _ _) (IterateI16Reply r) = Just $ lit r
parseQueryResult (IterateII32E _ _ _) (IterateI32Reply r) = Just $ lit r
parseQueryResult (IterateIIE _ _ _) (IterateI32Reply r) = Just $ lit (fromIntegral r)
parseQueryResult (IterateIL8E _ _ _) (IterateL8Reply r) = Just $ lit r
parseQueryResult (IterateIFloatE _ _ _) (IterateFloatReply r) = Just $ lit r
parseQueryResult (IterateL8UnitE _ _ _) (IterateUnitReply) = Just $ LitUnit
parseQueryResult (IterateL8BoolE _ _ _) (IterateBoolReply r) = Just $ lit r
parseQueryResult (IterateL8W8E _ _ _) (IterateW8Reply r) = Just $ lit r
parseQueryResult (IterateL8W16E _ _ _) (IterateW16Reply r) = Just $ lit r
parseQueryResult (IterateL8W32E _ _ _) (IterateW32Reply r) = Just $ lit r
parseQueryResult (IterateL8I8E _ _ _) (IterateI8Reply r) = Just $ lit r
parseQueryResult (IterateL8I16E _ _ _) (IterateI16Reply r) = Just $ lit r
parseQueryResult (IterateL8I32E _ _ _) (IterateI32Reply r) = Just $ lit r
parseQueryResult (IterateL8IE _ _ _) (IterateI32Reply r) = Just $ lit (fromIntegral r)
parseQueryResult (IterateL8L8E _ _ _) (IterateL8Reply r) = Just $ lit r
parseQueryResult (IterateL8FloatE _ _ _) (IterateFloatReply r) = Just $ lit r
parseQueryResult (IterateFloatUnitE _ _ _) (IterateUnitReply) = Just $ LitUnit
parseQueryResult (IterateFloatBoolE _ _ _) (IterateBoolReply r) = Just $ lit r
parseQueryResult (IterateFloatW8E _ _ _) (IterateW8Reply r) = Just $ lit r
parseQueryResult (IterateFloatW16E _ _ _) (IterateW16Reply r) = Just $ lit r
parseQueryResult (IterateFloatW32E _ _ _) (IterateW32Reply r) = Just $ lit r
parseQueryResult (IterateFloatI8E _ _ _) (IterateI8Reply r) = Just $ lit r
parseQueryResult (IterateFloatI16E _ _ _) (IterateI16Reply r) = Just $ lit r
parseQueryResult (IterateFloatI32E _ _ _) (IterateI32Reply r) = Just $ lit r
parseQueryResult (IterateFloatIE _ _ _) (IterateI32Reply r) = Just $ lit (fromIntegral r)
parseQueryResult (IterateFloatL8E _ _ _) (IterateL8Reply r) = Just $ lit r
parseQueryResult (IterateFloatFloatE _ _ _) (IterateFloatReply r) = Just $ lit r
