...
counterProg = do
      ...
      lcdBacklightOn lcd
      lcdWrite lcd $ show 0
      counter lcd getKey 0
  where
      countmod :: Key -> (Int -> Int)
      countmod KeyRight  = id
      countmod KeyLeft   = id
      countmod KeyUp     = (\x -> x + 1)
      countmod KeyDown   = (\x -> x - 1)
      countmod KeySelect = id
      counter lcd getKey count = do
          key <- getKey
          let count' = countmod key $ count
          lcdClear lcd
          lcdHome lcd
          lcdWrite lcd $ show count'
          counter lcd getKey count'
