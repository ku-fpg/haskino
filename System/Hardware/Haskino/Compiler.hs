...
compileProcedure (IterateUnitUnitE br iv bf) = do
    ...
    _ <- compileIterateProcedure UnitType UnitType b br i bi j bj iv bf
    return bj
compileProcedure (IterateUnitBoolE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindB j
    _ <- compileIterateProcedure UnitType BoolType b br i bi j bj iv bf
    return bj
compileProcedure (IterateUnitW8E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindW8 j
    _ <- compileIterateProcedure UnitType Word8Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateUnitW16E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindW16 j
    _ <- compileIterateProcedure UnitType Word16Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateUnitW32E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindW32 j
    _ <- compileIterateProcedure UnitType Word32Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateUnitI8E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI8 j
    _ <- compileIterateProcedure UnitType Int8Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateUnitI16E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI16 j
    _ <- compileIterateProcedure UnitType Int16Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateUnitI32E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI32 j
    _ <- compileIterateProcedure UnitType Int32Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateUnitIE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI j
    _ <- compileIterateProcedure UnitType IntType b br i bi j bj iv bf
    return bj
compileProcedure (IterateUnitL8E br iv bf) = do
    ...
    _ <- compileIterateProcedure UnitType List8Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateUnitFloatE br iv bf) = do
    ...
    _ <- compileIterateProcedure UnitType FloatType b br i bi j bj iv bf
    return bj
compileProcedure (IterateBoolUnitE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindUnit j
    _ <- compileIterateProcedure BoolType UnitType b br i bi j bj iv bf
    return bj
compileProcedure (IterateBoolBoolE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindB j
    _ <- compileIterateProcedure BoolType BoolType b br i bi j bj iv bf
    return bj
compileProcedure (IterateBoolW8E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindW8 j
    _ <- compileIterateProcedure BoolType Word8Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateBoolW16E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindW16 j
    _ <- compileIterateProcedure BoolType Word16Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateBoolW32E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindW32 j
    _ <- compileIterateProcedure BoolType Word32Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateBoolI8E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI8 j
    _ <- compileIterateProcedure BoolType Int8Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateBoolI16E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI16 j
    _ <- compileIterateProcedure BoolType Int16Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateBoolI32E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI32 j
    _ <- compileIterateProcedure BoolType Int32Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateBoolIE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI j
    _ <- compileIterateProcedure BoolType IntType b br i bi j bj iv bf
    return bj
compileProcedure (IterateBoolL8E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindList8 j
    _ <- compileIterateProcedure BoolType List8Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateBoolFloatE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindFloat j
    _ <- compileIterateProcedure BoolType FloatType b br i bi j bj iv bf
    return bj
compileProcedure (IterateW8UnitE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindUnit j
    _ <- compileIterateProcedure Word8Type UnitType b br i bi j bj iv bf
    return bj
compileProcedure (IterateW8BoolE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindB j
    _ <- compileIterateProcedure Word8Type BoolType b br i bi j bj iv bf
    return bj
compileProcedure (IterateW8W8E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindW8 j
    _ <- compileIterateProcedure Word8Type Word8Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateW8W16E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindW16 j
    _ <- compileIterateProcedure Word8Type Word16Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateW8W32E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindW32 j
    _ <- compileIterateProcedure Word8Type Word32Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateW8I8E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI8 j
    _ <- compileIterateProcedure Word8Type Int8Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateW8I16E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI16 j
    _ <- compileIterateProcedure Word8Type Int16Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateW8I32E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI32 j
    _ <- compileIterateProcedure Word8Type Int32Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateW8IE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI j
    _ <- compileIterateProcedure Word8Type IntType b br i bi j bj iv bf
    return bj
compileProcedure (IterateW8L8E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindList8 j
    _ <- compileIterateProcedure Word8Type List8Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateW8FloatE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindFloat j
    _ <- compileIterateProcedure Word8Type FloatType b br i bi j bj iv bf
    return bj
compileProcedure (IterateW16UnitE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindUnit j
    _ <- compileIterateProcedure Word16Type UnitType b br i bi j bj iv bf
    return bj
compileProcedure (IterateW16BoolE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindB j
    _ <- compileIterateProcedure Word16Type BoolType b br i bi j bj iv bf
    return bj
compileProcedure (IterateW16W8E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindW8 j
    _ <- compileIterateProcedure Word16Type Word8Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateW16W16E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindW16 j
    _ <- compileIterateProcedure Word16Type Word16Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateW16W32E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindW32 j
    _ <- compileIterateProcedure Word16Type Word32Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateW16I8E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI8 j
    _ <- compileIterateProcedure Word16Type Int8Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateW16I16E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI16 j
    _ <- compileIterateProcedure Word16Type Int16Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateW16I32E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI32 j
    _ <- compileIterateProcedure Word16Type Int32Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateW16IE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI j
    _ <- compileIterateProcedure Word16Type IntType b br i bi j bj iv bf
    return bj
compileProcedure (IterateW16L8E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindList8 j
    _ <- compileIterateProcedure Word16Type List8Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateW16FloatE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindFloat j
    _ <- compileIterateProcedure Word16Type FloatType b br i bi j bj iv bf
    return bj
compileProcedure (IterateW32UnitE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindUnit j
    _ <- compileIterateProcedure Word32Type UnitType b br i bi j bj iv bf
    return bj
compileProcedure (IterateW32BoolE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindB j
    _ <- compileIterateProcedure Word32Type BoolType b br i bi j bj iv bf
    return bj
compileProcedure (IterateW32W8E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindW8 j
    _ <- compileIterateProcedure Word32Type Word8Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateW32W16E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindW16 j
    _ <- compileIterateProcedure Word32Type Word16Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateW32W32E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindW32 j
    _ <- compileIterateProcedure Word32Type Word32Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateW32I8E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI8 j
    _ <- compileIterateProcedure Word32Type Int8Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateW32I16E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI16 j
    _ <- compileIterateProcedure Word32Type Int16Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateW32I32E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI32 j
    _ <- compileIterateProcedure Word32Type Int32Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateW32IE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI j
    _ <- compileIterateProcedure Word32Type IntType b br i bi j bj iv bf
    return bj
compileProcedure (IterateW32L8E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindList8 j
    _ <- compileIterateProcedure Word32Type List8Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateW32FloatE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindFloat j
    _ <- compileIterateProcedure Word32Type FloatType b br i bi j bj iv bf
    return bj
compileProcedure (IterateI8UnitE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindUnit j
    _ <- compileIterateProcedure Int8Type UnitType b br i bi j bj iv bf
    return bj
compileProcedure (IterateI8BoolE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindB j
    _ <- compileIterateProcedure Int8Type BoolType b br i bi j bj iv bf
    return bj
compileProcedure (IterateI8W8E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindW8 j
    _ <- compileIterateProcedure Int8Type Word8Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateI8W16E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindW16 j
    _ <- compileIterateProcedure Int8Type Word16Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateI8W32E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindW32 j
    _ <- compileIterateProcedure Int8Type Word32Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateI8I8E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI8 j
    _ <- compileIterateProcedure Int8Type Int8Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateI8I16E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI16 j
    _ <- compileIterateProcedure Int8Type Int16Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateI8I32E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI32 j
    _ <- compileIterateProcedure Int8Type Int32Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateI8IE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI j
    _ <- compileIterateProcedure Int8Type IntType b br i bi j bj iv bf
    return bj
compileProcedure (IterateI8L8E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindList8 j
    _ <- compileIterateProcedure Int8Type List8Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateI8FloatE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindFloat j
    _ <- compileIterateProcedure Int8Type FloatType b br i bi j bj iv bf
    return bj
compileProcedure (IterateI16UnitE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindUnit j
    _ <- compileIterateProcedure Int16Type UnitType b br i bi j bj iv bf
    return bj
compileProcedure (IterateI16BoolE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindB j
    _ <- compileIterateProcedure Int16Type BoolType b br i bi j bj iv bf
    return bj
compileProcedure (IterateI16W8E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindW8 j
    _ <- compileIterateProcedure Int16Type Word8Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateI16W16E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindW16 j
    _ <- compileIterateProcedure Int16Type Word16Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateI16W32E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindW32 j
    _ <- compileIterateProcedure Int16Type Word32Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateI16I8E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI8 j
    _ <- compileIterateProcedure Int16Type Int8Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateI16I16E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI16 j
    _ <- compileIterateProcedure Int16Type Int16Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateI16I32E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI32 j
    _ <- compileIterateProcedure Int16Type Int32Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateI16IE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI j
    _ <- compileIterateProcedure Int16Type IntType b br i bi j bj iv bf
    return bj
compileProcedure (IterateI16L8E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindList8 j
    _ <- compileIterateProcedure Int16Type List8Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateI16FloatE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindFloat j
    _ <- compileIterateProcedure Int16Type FloatType b br i bi j bj iv bf
    return bj
compileProcedure (IterateI32UnitE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindUnit j
    _ <- compileIterateProcedure Int32Type UnitType b br i bi j bj iv bf
    return bj
compileProcedure (IterateI32BoolE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindB j
    _ <- compileIterateProcedure Int32Type BoolType b br i bi j bj iv bf
    return bj
compileProcedure (IterateI32W8E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindW8 j
    _ <- compileIterateProcedure Int32Type Word8Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateI32W16E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindW16 j
    _ <- compileIterateProcedure Int32Type Word16Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateI32W32E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindW32 j
    _ <- compileIterateProcedure Int32Type Word32Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateI32I8E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI8 j
    _ <- compileIterateProcedure Int32Type Int8Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateI32I16E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI16 j
    _ <- compileIterateProcedure Int32Type Int16Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateI32I32E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI32 j
    _ <- compileIterateProcedure Int32Type Int32Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateI32IE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI j
    _ <- compileIterateProcedure Int32Type IntType b br i bi j bj iv bf
    return bj
compileProcedure (IterateI32L8E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindList8 j
    _ <- compileIterateProcedure Int32Type List8Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateI32FloatE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindFloat j
    _ <- compileIterateProcedure Int32Type FloatType b br i bi j bj iv bf
    return bj
compileProcedure (IterateIUnitE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindUnit j
    _ <- compileIterateProcedure IntType UnitType b br i bi j bj iv bf
    return bj
compileProcedure (IterateIBoolE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindB j
    _ <- compileIterateProcedure IntType BoolType b br i bi j bj iv bf
    return bj
compileProcedure (IterateIW8E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindW8 j
    _ <- compileIterateProcedure IntType Word8Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateIW16E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindW16 j
    _ <- compileIterateProcedure IntType Word16Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateIW32E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindW32 j
    _ <- compileIterateProcedure IntType Word32Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateII8E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI8 j
    _ <- compileIterateProcedure IntType Int8Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateII16E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI16 j
    _ <- compileIterateProcedure IntType Int16Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateII32E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI32 j
    _ <- compileIterateProcedure IntType Int32Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateIIE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI j
    _ <- compileIterateProcedure IntType IntType b br i bi j bj iv bf
    return bj
compileProcedure (IterateIL8E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindList8 j
    _ <- compileIterateProcedure IntType List8Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateIFloatE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindFloat j
    _ <- compileIterateProcedure IntType FloatType b br i bi j bj iv bf
    return bj
compileProcedure (IterateL8UnitE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindUnit j
    _ <- compileIterateProcedure List8Type UnitType b br i bi j bj iv bf
    return bj
compileProcedure (IterateL8BoolE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindB j
    _ <- compileIterateProcedure List8Type BoolType b br i bi j bj iv bf
    return bj
compileProcedure (IterateL8W8E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindW8 j
    _ <- compileIterateProcedure List8Type Word8Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateL8W16E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindW16 j
    _ <- compileIterateProcedure List8Type Word16Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateL8W32E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindW32 j
    _ <- compileIterateProcedure List8Type Word32Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateL8I8E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI8 j
    _ <- compileIterateProcedure List8Type Int8Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateL8I16E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI16 j
    _ <- compileIterateProcedure List8Type Int16Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateL8I32E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI32 j
    _ <- compileIterateProcedure List8Type Int32Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateL8IE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI j
    _ <- compileIterateProcedure List8Type IntType b br i bi j bj iv bf
    return bj
compileProcedure (IterateL8L8E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindList8 j
    _ <- compileIterateProcedure List8Type List8Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateL8FloatE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindFloat j
    _ <- compileIterateProcedure List8Type FloatType b br i bi j bj iv bf
    return bj
compileProcedure (IterateFloatUnitE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindUnit j
    _ <- compileIterateProcedure FloatType UnitType b br i bi j bj iv bf
    return bj
compileProcedure (IterateFloatBoolE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindB j
    _ <- compileIterateProcedure FloatType BoolType b br i bi j bj iv bf
    return bj
compileProcedure (IterateFloatW8E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindW8 j
    _ <- compileIterateProcedure FloatType Word8Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateFloatW16E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindW16 j
    _ <- compileIterateProcedure FloatType Word16Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateFloatW32E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindW32 j
    _ <- compileIterateProcedure FloatType Word32Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateFloatI8E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI8 j
    _ <- compileIterateProcedure FloatType Int8Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateFloatI16E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI16 j
    _ <- compileIterateProcedure FloatType Int16Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateFloatI32E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI32 j
    _ <- compileIterateProcedure FloatType Int32Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateFloatIE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindI j
    _ <- compileIterateProcedure FloatType IntType b br i bi j bj iv bf
    return bj
compileProcedure (IterateFloatL8E br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindList8 j
    _ <- compileIterateProcedure FloatType List8Type b br i bi j bj iv bf
    return bj
compileProcedure (IterateFloatFloatE br iv bf) = do
    ...
    j <- nextBind
    let bj = RemBindFloat j
    _ <- compileIterateProcedure FloatType FloatType b br i bi j bj iv bf
    return bj
